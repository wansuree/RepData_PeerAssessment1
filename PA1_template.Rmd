...
a <- rbind(cbind(avg_weekday, intervals, "weekday"), 
           cbind(avg_weekend, intervals, "weekend"))

a <- data.frame(a)
names(a) <- c("steps", "interval", "day")

library(lattice)
xyplot(steps ~ interval | day, data = a, layout = c(1, 2), 
       type = "l")
library(ggplot2)
qplot(interval, steps, data = a, facets = .~day, type = "l")
```
